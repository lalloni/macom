...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ''' Tomo todas las interfaces no publicadas aun '''
        for d in orm.Dependency.objects.filter(interface__published = False):
            ''' Busca las dependencias que utilicen una interface de otro sistema '''
            if d.module.system != d.interface.module.system:
                ''' modifico el estado de la publicacion de la interface '''
                d.interface.published = True
                d.interface.save()
